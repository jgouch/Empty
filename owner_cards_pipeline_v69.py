#!/usr/bin/env python3
"""Owner Card Pipeline — v69 (FaCTS Section Validation + Full Snapshots + NeedsReviewNotes)
...
Run:
  python3 owner_cards_pipeline_v69.py
  or
  python3 owner_cards_pipeline_v69.py --pdf "B (all).pdf" --out "OwnerCards_B_Output.xlsx"
...
def _auto_pick_facts(pdf_path: str) -> str:
    """v69: If --facts not provided, try to find a likely FaCTS export near the PDF."""
    try:
        base_dir = os.path.dirname(os.path.abspath(pdf_path)) if pdf_path else os.getcwd()
        cands = []
        for pat in ('*.xlsx', '*.xlsm', '*.xls'):
            cands.extend(glob.glob(os.path.join(base_dir, pat)))
        # prefer names that look like the inventory export
        pref = [p for p in cands if re.search(r'(facts|inventory|property\s*-\s*property\s*inventory|inventory\s*listing|property\s*inventory)', os.path.basename(p), re.IGNORECASE)]
        pool = pref if pref else cands
        if not pool:
            return ''
        pool.sort(key=lambda x: os.path.getmtime(x), reverse=True)
        return pool[0]
    except Exception:
        return ''
...
# v69: base directory for snapshot outputs (set to PDF folder during processing)
SNAPSHOT_BASE_DIR = ""
# v69: rotate snapshots to landscape for readability (does NOT affect OCR)
LANDSCAPE_LOCK_SNAPSHOTS = True
...
def match_facts_section_from_line(line_text: str, facts_sections: List[str]) -> Tuple[str, float, str]:
    ...
    if m:
        ...
        cand = re.split(r'\bLOT\b|\bSEC\b|\bSECTION\b|\bSPACE\b|\d', cand, 1)[0].strip()
    ...
    if property_lines:
        if facts_sections:
            ...
            if unmatched_lines and not matched_sections:
                ...
                needs_review = True
                notes.append(f'PROPERTY_SECTION_PARTIAL_{len(matched_sections)}of{len(property_lines)}')
        else:
...
def _auto_rotate_by_header_score(pil_img: Image.Image, header_ratio: float = 0.35) -> tuple[Image.Image, dict]:
    """Try rotations and pick the one with the best header OCR score."""
    best = pil_img
    best_meta = {"angle": 0, "score": -1, "ratio": 0.0}
    for angle in (0, 90, 180, 270):
        try:
            ...
            try:
                t = pytesseract.image_to_string(header_pp, config='--oem 3 -l eng --psm 6')
            except Exception:
                t = ''
            sc = score_header_text(t)
            # v65: tie-break orientation when OCR scores are close: prefer landscape (w/h)
            ratio = (w / max(h, 1))
            best_score = best_meta.get('score', -1)
            best_ratio = best_meta.get('ratio', 0.0)
            if (sc > best_score) or (sc == best_score and ratio > best_ratio + 0.10) or (sc == best_score and abs(ratio-best_ratio) <= 0.10 and angle < best_meta.get('angle', 0)):
                best = cand
                best_meta = {"angle": angle, "score": sc, "ratio": ratio}
...
def _landscape_lock(pil_img: Image.Image) -> Image.Image:
    """v69: rotate snapshot to landscape if it is taller than wide. For readability only."""
    try:
        if not LANDSCAPE_LOCK_SNAPSHOTS:
            return pil_img
        w, h = pil_img.size
        if h > w:
...
def save_failure_snapshots(pil_img: Image.Image, source_pdf: str, page_num: int, reason: str = "NEEDS_REVIEW") -> Dict[str, str]:
    """v69: Save ONLY a full-page snapshot for manual verification.

    IMPORTANT:
      - This function never runs OSD/deskew/dewarp.
      - It may optionally rotate to landscape for readability (LANDSCAPE_LOCK_SNAPSHOTS).
    """
    ...
    img = pil_img
    try:
        img = _landscape_lock(img)
    except Exception:
        pass

    try:
        img.save(full_path, quality=92)
    except Exception:
        img.convert('RGB').save(full_path, quality=92)

    def _rel(pth: str) -> str:
        try:
            if SNAPSHOT_BASE_DIR:
                return os.path.relpath(pth, start=SNAPSHOT_BASE_DIR)
        except Exception:
            pass
        return pth
...
# v69: structural property patterns (older cards)
STRUCT_PROP_RE = re.compile(r"\\b\\d{1,4}\\s*[:/\\-]\\s*[A-Z]\\s*[:/\\-]\\s*\\d{1,4}\\b")
...
# v69: detect handwritten / faint X marks in rightmost column for a given line band
...
def preprocess_ghost(pil_img: Image.Image) -> Image.Image:
    ...
    bin_img = cv2.erode(bin_img, kernel, iterations=1)  # v69: thicken faint ink
...
def classify_item(line: str) -> Dict[str, bool]:
    t = line or ""
    flags = {
        "IsProperty": bool(re.search(r"(space|sp\.?|lot|section|sec\.?|block|blk\.?|garden|crypt|lawn|grave|burial|mausoleum|maus\.?|niche|columbarium|estates?)", t, re.IGNORECASE) or (t.count(':') >= COLON_COUNT_PROP_MIN and ( (GARDEN_NORM_RE.search(normalize_section_name(t)) if 'GARDEN_NORM_RE' in globals() and GARDEN_NORM_RE else re.search(r'(PRAYER|LAST SUPPER|GETHSEMANE|CHAPEL HILL|PEACE|FAITH|SERENITY|FOUNTAIN|EVERLASTING LIFE|GOOD SHEPHERD|CROSS|ATONEMENT)', t, re.IGNORECASE)) or STRUCT_PROP_RE.search(t) )) ),
...
# v69: quick scoring for autorotation fallback
def evaluate_ocr_quality_quick(text: str) -> int:
    if not text:
        return -999
    score = 0
    try:
        if re.search(ZIP_RE, text):
            score += 10
    except Exception:
        pass
    try:
        top = " ".join(split_lines(text)[:8])
        if re.search(r"[A-Z]{2,}", top):
            score += 3
        if re.search(r"[AEIOUYaeiouy]", top):
            score += 2
        if 'OWNER' in (text.upper()):
            score += 3
        if 'ITEM DESCRIPTION' in (text.upper()):
            score += 3
    except Exception:
        pass
    return int(score)
...
def autorotate_pil(pil_img: Image.Image) -> Tuple[Image.Image, Dict]:
    """v69: try OSD; if weak/failed, brute-force 0/90/180/270 using quick OCR on a small header sample."""
    meta = {'method': 'none', 'rotate': 0, 'osd_conf': 0, 'score': -999}
    if pil_img is None:
        return pil_img, meta
    ...
    # --- Try OSD first ---
    try:
        osd = pytesseract.image_to_osd(preprocess_high_contrast(img_rgb))
        ...
        if rot in (90, 180, 270) and conf >= 5:
            meta['method'] = 'osd'
            return img_rgb.rotate(360-rot, expand=True), meta
    except Exception:
        pass
    ...
    try:
        w0, h0 = img_rgb.size
        # header sample: top 35%
        for deg in (0, 90, 180, 270):
            test = img_rgb.rotate(deg, expand=True) if deg else img_rgb
            w, h = test.size
            header = test.crop((0, 0, w, int(h*0.35)))
            small = header.copy()
            small.thumbnail((1100, 1100))
            txt = pytesseract.image_to_string(preprocess_standard(small), config='--oem 3 -l eng --psm 6')
            sc = evaluate_ocr_quality_quick(txt)
            if sc > best_score:
                best_score = sc
                best = test
                best_deg = deg
        meta['method'] = 'bruteforce'
        meta['rotate'] = best_deg
        meta['score'] = int(best_score)
    except Exception:
        pass
    return best, meta
    ...
    for tag, img_v in variants:
        for psm in psm_list:
            try:
                ...
                t = pytesseract.image_to_string(img_v, config=cfg)
            except Exception:
                t = ''
            used['tesseract'] = True
            sc = score_header_text(t)
...
def process_page(pdf_path: str, page_index: int, dpi: int, target_char: Optional[str], reader: Optional[PdfReader] = None, kraken_model: str = '', kraken_bin: str = 'kraken', kraken_python: str = '', allow_livetext: bool = True, facts_sections: Optional[List[str]] = None, facts_mode: str = '', alt_ocr: str = "ocrmac_then_kraken") -> Tuple[Dict, List[Dict], bool]:
            ...
            try:
                if (not phone_fields.get('PhoneValid')) and (not (phone_fields.get('PhoneRaw') or phone_fields.get('Phone') or phone_fields.get('PhoneNormalized'))):
                    ...
                    if pil_h is not None:
                        pil_h, _m = normalize_for_ocr(pil_h)
    ...
    # OCR FALLBACK (full)
    pil_snapshot_raw = render_page(pdf_path, page_index, dpi=dpi)
    pil_original = pil_snapshot_raw.copy() if pil_snapshot_raw is not None else None
    ...
    owner.update(phone_fields)
    owner = enrich_owner_with_review(owner, items, pdf_path, page_index+1, dpi=dpi, pil_page=pil_snapshot_raw, facts_sections=facts_sections, facts_mode=facts_mode)
    return owner, items, is_interment
...
def process_dataset(pdf_path: str, out_path: str, dpi: int = 300, kraken_model: str = '', kraken_bin: str = 'kraken', kraken_python: str = '', allow_livetext: bool = True, facts_path: str = '', alt_ocr: str = "ocrmac_then_kraken"):
    ...
    # v69: write snapshots next to the input PDF folder
    global FAILED_SNAPSHOT_DIR, SNAPSHOT_BASE_DIR
    try:
        SNAPSHOT_BASE_DIR = os.path.dirname(os.path.abspath(pdf_path))
        FAILED_SNAPSHOT_DIR = os.path.join(SNAPSHOT_BASE_DIR, '_Failed_Snapshots')
    except Exception:
        SNAPSHOT_BASE_DIR = ''
        FAILED_SNAPSHOT_DIR = '_Failed_Snapshots'
    ...
    def agg_owner(group: pd.DataFrame) -> pd.Series:
        has_property_strict = bool(group.get("IsProperty", False).any())
        # v69: loose property evidence: any IsProperty OR any RightsNotation OR any fallback_text_scan lines
        try:
            loose = bool(group.get("IsProperty", False).any())
            if not loose:
                loose = bool(group.get("HasRightsNotation", False).any())
            if not loose and "LineSource" in group.columns:
                loose = bool((group["LineSource"].fillna("") == "fallback_text_scan").any())
        except Exception:
            loose = False
        has_property_loose = bool(loose)
        has_property = bool(has_property_strict or has_property_loose)
        ...
        return pd.Series({
            ...
            "HasProperty_STRICT": bool(has_property_strict),
            "HasProperty_LOOSE": bool(has_property_loose),
    ...
    defaults = {
        "HasProperty_STRICT": False,
        "HasProperty_LOOSE": False,
...
def main():
    ...
    if not args.facts and args.pdf:
        try:
            args.facts = _auto_pick_facts(args.pdf)
        except Exception:
            pass
