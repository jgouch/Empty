...
def detect_strike_segs_for_page(pdf_path: str, page_index: int, dpi: int = 150) -> List[Tuple[int, int, int, int]]:
...
def ocr_items_with_strike(pdf_path: str, page_index: int, dpi: int, strike_segs: List[Tuple[int, int, int, int]]) -> List[Dict]:
    """OCR the page in data mode (std+clahe), generate item_dict_from_line with struck flags."""
    pil_img = render_page(pdf_path, page_index, dpi=dpi)
    if pil_img is None:
        return []
    ...
    def add_lines(lines_ocr):
        for ln_obj in lines_ocr:
            ...
            items.append(item_dict_from_line(txt, struck=struck))
            seen.add(key)
...
def parse_best_address(lines: List[str]) -> Dict:
    ...
    if not candidates:
        ...
        for i, line in enumerate(scan):
            ...
            # Must contain letters (avoid numeric/table artifacts)
            if not any(ch.isalpha() for ch in cleaned):
                continue

            # Score: earlier is better, and street suffix gets a bonus
            score = 100 - i
            ...
            # Penalize commas a bit (often 'street, city ...' that we failed to parse)
            if "," in cleaned:
                score -= 5
...
def process_page(pdf_path: str, page_index: int, dpi: int, target_char: Optional[str], reader: Optional[PdfReader] = None) -> Tuple[Dict, List[Dict], bool]:
    ...
    if text_layer_usable(pdf_text):
        ...
        if ok:
            ...
            strike_segs = detect_strike_segs_for_page(pdf_path, page_index, dpi=min(150, max(110, dpi//2)))
            if strike_segs:
                # Build items via OCR so struck lines are excluded consistently
                items = [] if is_interment else ocr_items_with_strike(pdf_path, page_index, dpi=min(220, dpi), strike_segs=strike_segs)
            else:
