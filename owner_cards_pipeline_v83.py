#!/usr/bin/env python3
"""Owner Card Pipeline — v83 (Line Stitching + FaCTS Space Fallback + Notes OCR Lists)
...
Run:
  python3 owner_cards_pipeline_v83.py
  or
  python3 owner_cards_pipeline_v83.py --pdf "B (all).pdf" --out "OwnerCards_B_Output.xlsx"
...
# ------------------------------
# v77 additions: OCR-tolerant ZIP/State + Evidence + Page gate + Header line-groups + FaCTS backfill
# ------------------------------

OCR_ZIP_SWAP = str.maketrans({
    'O': '0', 'Q': '0', 'D': '0',
    'I': '1', 'L': '1', '|': '1',
    'S': '5', '$': '5',
    'B': '8',
...
NEAR_ZIP_RE = re.compile(r"\b[0-9OQDIILSZBGTX]{5}\b", re.IGNORECASE)

def normalize_zip_token(tok: str) -> str:
    if not tok:
        return ''
    t = re.sub(r"[^0-9A-Z]", "", str(tok).upper()).translate(OCR_ZIP_SWAP)
    t = re.sub(r"[^0-9]", "", t)
    return t[:5] if len(t) >= 5 else t

def rescue_zip(text: str) -> str:
    """OCR-tolerant ZIP rescue: normalize common OCR swaps and return 5 digits."""
    if not text:
        return ''
    u = str(text).upper()
    for m in NEAR_ZIP_RE.finditer(u):
        z = normalize_zip_token(m.group(0))
        if re.fullmatch(r"\d{5}", z):
            return z
    return ''

TN_ZIP_PREFIXES = ('37', '38')
TN_CITY_HINTS = {
    'NASHVILLE','FRANKLIN','BRENTWOOD','FAIRVIEW','PEGRAM','DICKSON',
    'GOODLETTSVILLE','HENDERSONVILLE','KINGSTON SPRINGS','WHITE BLUFF',
    'BURNS','CLARKSVILLE','MADISON','ANTIOCH','SMYRNA','MURFREESBORO',
}

def normalize_state_token(state: str, city: str = '', zip5: str = '') -> str:
    """Fix OCR TN↔IN confusion without corrupting real Indiana."""
    if not state:
        return ''
    st = re.sub(r"[^A-Z0-9]", "", str(state).upper())
    if st == '1N':
        st = 'TN'

    if st == 'IN':
        cz = str(zip5 or '')
        c = str(city or '').upper().strip()
        if cz.startswith(TN_ZIP_PREFIXES) or c in TN_CITY_HINTS:
            return 'TN'
        return 'IN'
    return st

def fix_target_letter(name: str, target: str) -> str:
    """Fuzzy first-letter correction: B→8, C→([<{"""
    if not name or not target:
        return name or ''
    t = str(target).upper().strip()[:1]
    s = str(name).lstrip()
    if not s:
        return ''
    if t == 'B' and s[0] == '8':
...
# Evidence extraction (run for ALL cards, not only NeedsReview)
TRANSFER_ANY_RE = re.compile(r"\bTRANSF(?:ER|ERS|ERRED|ERED)\b", re.IGNORECASE)
TRANSFER_ALL_TO_RE = re.compile(r"\bTRANSF(?:ERRED|ERED)?\s+ALL\s+TO\s+([A-Z][A-Z \-\']{2,60})\b", re.IGNORECASE)
DEL_ANY_RE = re.compile(r"\bDEL\b|\bDELIV(?:ERED|ERY)\b|\bPN\s*DEL\b", re.IGNORECASE)
UNSEL_ANY_RE = re.compile(r"\bUN\s*-?\s*SELECT(?:ED|ION)?\b|\bUNSELECTED\b", re.IGNORECASE)
RESELECT_RE = re.compile(r"\bRE\s*SELECT(?:ED|ION)?\b|\bRESELECT\b", re.IGNORECASE)
DATE_ANY_RE = re.compile(r"\b(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})\b")

def extract_action_evidence(blob: str) -> dict:
    out = {
        'IsTransferredOutEvidence': False,
        'IsDeliveredEvidence': False,
        'IsUnselectedEvidence': False,
        'IsReselectedEvidence': False,
        'TransferToEvidence': '',
        'TransferDateEvidence': '',
    }
    if not blob:
        return out
    s = str(blob)
    out['IsTransferredOutEvidence'] = bool(TRANSFER_ANY_RE.search(s))
    out['IsDeliveredEvidence'] = bool(DEL_ANY_RE.search(s))
    out['IsUnselectedEvidence'] = bool(UNSEL_ANY_RE.search(s))
    out['IsReselectedEvidence'] = bool(RESELECT_RE.search(s))
    m_to = TRANSFER_ALL_TO_RE.search(s.upper())
    if m_to:
        out['TransferToEvidence'] = m_to.group(1).strip()
    m_dt = DATE_ANY_RE.search(s)
    if m_dt:
        out['TransferDateEvidence'] = m_dt.group(1)
    return out

def card_confidence_score(owner: dict, items: list, raw_text: str) -> int:
    """Confidence score used for triage + skip gating."""
    blob = (raw_text or '') + "\n" + "\n".join([str(it.get('LineText','') or '') for it in (items or [])])
    zip5 = owner.get('ZIP','') or rescue_zip(blob)
    name = str(owner.get('PrimaryOwnerName','') or '')
    street = str(owner.get('Street','') or '')
    phone = str(owner.get('PhoneNormalized','') or '')

    score = 0
    if name and '[MISSING' not in name.upper() and not is_gibberish(name):
        score += 3
    if zip5 and re.fullmatch(r"\d{5}", str(zip5)):
        score += 3
    if street and re.search(STREET_START_RE, street):
        score += 2
    if re.search(r"\bLOT\b|\bSEC\b|\bSECTION\b|\bSP\b|\bSPACE\b|\bCRLC\b|\bL/C\b|\bLAWN\s*CRYPT\b|\bNICHE\b|\bMAUS\b", blob.upper()):
        score += 2
    if phone and re.search(r"\d", phone):
        score += 1
    return score

def page_is_card(owner: dict, items: list, raw_text: str) -> tuple[bool, str, int]:
    # v78: hard skips
    try:
        if str(owner.get('TemplateType','')) == 'interment_record':
            return (False, 'INTERMENT_RECORD', 0)
        if bool(owner.get('IsBlankSeparator', False)):
            return (False, 'BLANK_SEPARATOR', 0)
        if 'BLANK' in str(owner.get('OwnerName_Raw','')).upper():
            return (False, 'BLANK_OWNERNAME_RAW', 0)
    except Exception:
        pass

    sc = card_confidence_score(owner, items, raw_text)
    if sc < 2:
        return (False, 'NO_CARD_SIGNALS', sc)
    return (True, '', sc)

# Header OCR line-group parsing (image_to_data) on a top crop
def ocr_header_lines(pil_page: Image.Image, header_ratio: float = 0.40) -> list[str]:
    if pil_page is None:
        return []
    w, h = pil_page.size
    crop = pil_page.crop((0, 0, w, int(h * header_ratio)))

    variants = []
    try:
        variants.append(preprocess_shadow_correct(crop))
    except Exception:
        variants.append(crop.convert('L'))
    try:
        variants.append(preprocess_clahe(crop))
    except Exception:
        pass
    ...
    for img in variants:
        for psm in (6, 4, 11):
            data = _tess_image_to_data(img, config=f"{TESS_CONFIG_BASE} --psm {psm}")
            ...
            lines = [' '.join(buckets[k]) for k in sorted(buckets.keys())]
            sc = score_header_text("\n".join(lines))
            if sc > best_score:
                best_score = sc
                best_lines = lines

    return [normalize_ws(x) for x in best_lines if x and not is_gibberish(x)]

def parse_header_from_lines(lines: list[str]) -> dict:
    out = {'PrimaryOwnerName': '', 'Street': '', 'City': '', 'State': '', 'ZIP': '', 'AddressRaw': ''}
    if not lines:
        return out
    ...
    for i, ln in enumerate(lines):
        m = re.search(ZIP_RE, ln)
        z = m.group(0)[:5] if m else rescue_zip(ln)
        if z:
            zip_line_idx = i
            zip5 = z
            break

    if zip_line_idx is not None:
        ln = lines[zip_line_idx]
        city, st, z = extract_zip_state(ln)
        if not z:
            z = zip5

        out['ZIP'] = z
        out['State'] = normalize_state_token(st, city, z)
        out['City'] = city

        if zip_line_idx - 1 >= 0:
            out['Street'] = lines[zip_line_idx - 1]

        out['AddressRaw'] = normalize_ws(' '.join([out['Street'], out['City'], out['State'], out['ZIP']]).strip())

        for j in range(max(0, zip_line_idx - 4), zip_line_idx):
            cand = lines[j]
            if cand and not is_gibberish(cand):
                if re.search(r"\bOWNER\b|\bSINCE\b|\bITEM\b\s+DESC", cand, re.IGNORECASE):
                    continue
                out['PrimaryOwnerName'] = cand
                break
    else:
        for ln in lines[:3]:
            if ln and not is_gibberish(ln):
                out['PrimaryOwnerName'] = ln
                break

    return out

def build_facts_owner_index(facts_df: pd.DataFrame) -> dict:
    """Map (lotsec, sp) -> best owner/contact info from FaCTS export.

    v79:
      - Correctly parses 'Lot/Section <TOKEN> Space <TOKEN>' with alphanumeric and range/comma support.
      - Stores multiple keys per row (clean + OCR-tolerant, plus expanded numeric ranges/lists).
      - Uses itertuples() for performance on large exports.
    """
    if facts_df is None or facts_df.empty:
        return {}

    cols = {str(c).strip().lower(): c for c in facts_df.columns}
    space_col = cols.get('space') or next((v for k, v in cols.items() if k == 'space' or ' space' in k), None)
    owner_col = cols.get('owner name') or next((v for k, v in cols.items() if 'owner name' in k), None)
    addr_col = cols.get('owner address') or next((v for k, v in cols.items() if 'owner address' in k), None)
    city_col = cols.get('owner city') or next((v for k, v in cols.items() if 'owner city' in k), None)
    st_col = cols.get('owner state') or next((v for k, v in cols.items() if 'owner state' in k), None)
    zip_col = cols.get('owner postal code') or next((v for k, v in cols.items() if 'owner postal' in k or 'postal code' in k), None)
    if space_col is None:
        return {}

    use_cols = [c for c in [space_col, owner_col, addr_col, city_col, st_col, zip_col] if c is not None]
    df2 = facts_df[use_cols].fillna('')

    pos = {c: i for i, c in enumerate(use_cols)}
    p_space = pos.get(space_col)
    p_owner = pos.get(owner_col) if owner_col is not None else None
    p_addr = pos.get(addr_col) if addr_col is not None else None
    p_city = pos.get(city_col) if city_col is not None else None
    p_st = pos.get(st_col) if st_col is not None else None
    p_zip = pos.get(zip_col) if zip_col is not None else None

    out = {}
    facts_re = re.compile(r"LOT/SECTION\s*([0-9A-Z]+)\s*SPACE\s*([0-9A-Z]+(?:[-,][0-9A-Z]+)*)", re.IGNORECASE)

    for row in df2.itertuples(index=False, name=None):
        sptext = str(row[p_space] or '').strip()
        if not sptext:
            continue
        m = facts_re.search(sptext.upper())
        if m:
            lotsec = m.group(1).strip()
            sp_raw = m.group(2).strip()
        else:
            lotsec, _, sp_raw = parse_lotsec_sp(sptext)
        if not lotsec or not sp_raw:
            continue

        tokens = [sp_raw] + _expand_space_token(sp_raw)
        keys = []
        for tok in tokens:
            keys.append((lotsec, _norm_sp_facts(tok)))
            keys.append((lotsec, _norm_sp(tok)))
        seen=set(); uniq_keys=[]
        for k in keys:
            if k not in seen:
                seen.add(k); uniq_keys.append(k)

        info = {
            'OwnerName': str(row[p_owner]).strip() if p_owner is not None else '',
            'OwnerAddress': str(row[p_addr]).strip() if p_addr is not None else '',
            'OwnerCity': str(row[p_city]).strip() if p_city is not None else '',
            'OwnerState': str(row[p_st]).strip() if p_st is not None else '',
            'OwnerZIP': normalize_zip_token(str(row[p_zip])) if p_zip is not None else '',
        }
...
def load_facts_sections(facts_path: str) -> List[str]:
    """Load unique FaCTS Section values (Garden names) from an inventory export.

    v75: auto-detects the header row (handles title rows above the header) and then
    reads the 'Section' column by name (not position). Designed for the
    'Property - Property Inventory with Owner Details' export.
    """
    if not facts_path or not os.path.exists(facts_path):
        return []
    try:
        preview = pd.read_excel(facts_path, engine='openpyxl', sheet_name=0, header=None, nrows=30)
        header_row = 0
        for i in range(min(len(preview), 30)):
            vals = [str(x).strip().lower() for x in preview.iloc[i].tolist() if str(x) != 'nan']
            if 'section' in vals and 'space' in vals and 'owner name' in vals:
                header_row = i
                break
        df = pd.read_excel(facts_path, engine='openpyxl', sheet_name=0, header=header_row)
        ...
        if sec_col is None:
            for c in df.columns:
                if 'section' in str(c).strip().lower():
                    sec_col = c
                    break
        if sec_col is None:
            return []
        secs = df[sec_col].dropna().astype(str).str.strip()
        seen = set(); out_list = []
        for s in secs.tolist():
            if not s or s.lower() == 'section':
                continue
            if s not in seen:
...
def enrich_owner_with_review(owner: Dict, items: List[Dict], pdf_path: str, page_num: int, dpi: int, pil_page: Optional[Image.Image] = None, facts_sections: Optional[List[str]] = None, facts_mode: str = '', reader: Optional[PdfReader] = None, facts_space_index: Optional[dict] = None, facts_owner_index: Optional[dict] = None, facts_maus_index: Optional[dict] = None) -> Dict:
    ...
    # v77: action evidence from RawText + items for ALL pages
    blob = owner.get('RawText','') + '\n' + '\n'.join([it.get('LineText','') for it in items])
    ev = extract_action_evidence(blob)
    owner['IsTransferredOutEvidence'] = ev.get('IsTransferredOutEvidence', False)
    owner['IsDeliveredEvidence'] = ev.get('IsDeliveredEvidence', False)
    owner['IsUnselectedEvidence'] = ev.get('IsUnselectedEvidence', False)
    owner['IsReselectedEvidence'] = ev.get('IsReselectedEvidence', False)
    owner['TransferToEvidence'] = ev.get('TransferToEvidence', '')
    owner['TransferDateEvidence'] = ev.get('TransferDateEvidence', '')
    tl = os.path.basename(pdf_path)[:1].upper()
    owner['PrimaryOwnerName'] = fix_target_letter(owner.get('PrimaryOwnerName', ''), tl)
    needs_review = False
    ...
    for it in (items or []):
        try:
            txt = (it.get('LineText', '') or '').strip()
            u = txt.upper()
            looks_like_modern_coords = bool(re.search(r"\b\d{1,4}[A-Z]?\b\s*\b\d{1,2}\b$", u))
            looks_like_colon_coords = (txt.count(':') >= 2)
            if it.get('IsProperty') or (it.get('RightsNotation') or '') or looks_like_modern_coords or looks_like_colon_coords:
                property_lines.append(txt)
        except Exception:
            continue
    ...
    # v76: stitch split property lines (Sec/Sp line above Lot line, etc.)
    try:
        property_lines = stitch_property_lines(property_lines)
    except Exception:
        pass


    # v80: structured property parsing + FaCTS match scoring for each item
    try:
        if facts_sections is not None and isinstance(facts_sections, list) and facts_sections:
            facts_index = build_facts_section_index(facts_sections)
        else:
            facts_index = []
    except Exception:
        facts_index = []
    ...
    for it in (items or []):
        try:
            ...
            if not txt:
                continue
            # Only parse/match for likely property candidates
            if not (it.get('IsProperty') or (it.get('RightsNotation') or '') or txt.count(':') >= 2 or parse_modern_lotsec_sp(txt)[0]):
                continue

            lotsec, sec_only, sp = parse_lotsec_sp(txt)
            if not (lotsec and sp):
                lotsec2, sp2 = parse_modern_lotsec_sp(txt)
                if lotsec2 and sp2:
                    lotsec, sp = lotsec2, sp2
                    sec_only = re.sub(r'^[0-9]+', '', lotsec2)

            if lotsec and sp:
                it['Prop_LotSection'] = lotsec
                it['Prop_Section'] = sec_only or ''
                it['Prop_Space'] = sp
                it['Prop_RawKey'] = f"{lotsec}:{sp}"

            # Infer garden/section name
            sec_name = ''
            conf = 0.0
            method = ''
            if facts_sections:
                sec_name, conf, method = match_facts_section_from_line(txt, facts_sections, facts_index=facts_index)
            if (not sec_name) and (facts_space_index is not None) and lotsec and sp:
                sec2, conf2, method2 = infer_section_from_facts_space(txt, facts_space_index)
                if sec2:
                    sec_name, conf, method = sec2, conf2, method2
            if sec_name:
                it['Prop_Garden'] = sec_name

            # FaCTS match scoring on (lotsec, sp)
            if (facts_space_index is not None) and lotsec and sp:
                cands = list(facts_space_index.get((lotsec, sp), set()))
                if len(cands) == 1:
                    it['FaCTS_MatchConfidence'] = 0.90
                    it['FaCTS_MatchMethod'] = 'facts_space_unique'
                    it['FaCTS_MatchedSections'] = cands[0]
                    it['FaCTS_MatchedSpace'] = f"{lotsec}:{sp}"
            # v83: Maus/Niche FaCTS matching (C/L / Row / Depth / Niche)
            if (not (lotsec and sp)) and (facts_maus_index is not None):
                mk = maus_component_key(txt) or normalize_maus_text(txt)
                if mk:
                    entries = list(facts_maus_index.get(mk, []))
                    if entries:
                        secs = sorted({str(e.get('Section','')).strip() for e in entries if str(e.get('Section','')).strip()})
                        it['FaCTS_MatchedSections'] = '; '.join(secs)
                        primary = entries[0]
                        it['FaCTS_MatchedSpace'] = str(primary.get('Space',''))
                        it['FaCTS_MatchedRowIndex'] = str(primary.get('RowIndex',''))
                        it['FaCTS_MatchConfidence'] = 0.95 if len(entries) == 1 else 0.75
                        it['FaCTS_MatchMethod'] = 'facts_maus_unique' if len(entries) == 1 else 'facts_maus_multi'
                        it['Prop_Type'] = it.get('Prop_Type') or 'maus_niche'

                        if (('[MISSING' in prim.upper()) or (not owner.get('Street')) or (not owner.get('ZIP'))):
                            if primary.get('OwnerName') and ('[MISSING' in prim.upper() or not owner.get('PrimaryOwnerName')):
                                owner['PrimaryOwnerName'] = primary.get('OwnerName')
                                owner['BackfillSource'] = 'FaCTS_Maus_Backfill'
                            if primary.get('OwnerAddress') and (not owner.get('Street')):
                                owner['Street'] = normalize_street(primary.get('OwnerAddress'))
                                owner['BackfillSource'] = 'FaCTS_Maus_Backfill'
                            if primary.get('OwnerCity') and (not owner.get('City')):
                                owner['City'] = primary.get('OwnerCity')
                                owner['BackfillSource'] = 'FaCTS_Maus_Backfill'
                            if primary.get('OwnerState') and (not owner.get('State')):
                                owner['State'] = normalize_state_token(primary.get('OwnerState',''), owner.get('City',''), owner.get('ZIP',''))
                                owner['BackfillSource'] = 'FaCTS_Maus_Backfill'
                            if primary.get('OwnerZIP') and (not owner.get('ZIP')):
                                owner['ZIP'] = normalize_zip_token(primary.get('OwnerZIP',''))
                                owner['BackfillSource'] = 'FaCTS_Maus_Backfill'
                elif len(cands) > 1:
                    it['FaCTS_MatchConfidence'] = 0.70
                    it['FaCTS_MatchMethod'] = 'facts_space_multi'
                    it['FaCTS_MatchedSections'] = '; '.join(sorted(set(cands)))
                    it['FaCTS_MatchedSpace'] = f"{lotsec}:{sp}"
                else:
                    it['FaCTS_MatchConfidence'] = 0.0
                    it['FaCTS_MatchMethod'] = 'no_match'
                    it['FaCTS_MatchedSpace'] = f"{lotsec}:{sp}" if (lotsec and sp) else ''
        except Exception:
            continue

    # v80: backfill using structured property keys (prefer parsed item tokens)
    if facts_owner_index and (('[MISSING' in prim.upper()) or (not street.strip())):
        backfilled = False
        # 1) structured item keys
        for it in (items or []):
            try:
                lotsec = str(it.get('Prop_LotSection', '') or '').strip()
                sp = str(it.get('Prop_Space', '') or '').strip()
                if not (lotsec and sp):
                    lotsec, _, sp = parse_lotsec_sp(it.get('LineText', '') or '')
                if not (lotsec and sp):
                    lotsec, sp = parse_modern_lotsec_sp(it.get('LineText', '') or '')
                if not (lotsec and sp):
                    continue
                key = (lotsec, _norm_sp(sp))
                if key in facts_owner_index:
                    info = facts_owner_index.get(key, {})
                    if '[MISSING' in prim.upper() or not owner.get('PrimaryOwnerName'):
                        owner['PrimaryOwnerName'] = info.get('OwnerName', '') or owner.get('PrimaryOwnerName', '')
                    if not owner.get('Street'):
                        owner['Street'] = info.get('OwnerAddress', '') or owner.get('Street', '')
                    if not owner.get('City'):
                        owner['City'] = info.get('OwnerCity', '') or owner.get('City', '')
                    if not owner.get('State'):
                        owner['State'] = normalize_state_token(info.get('OwnerState', ''), owner.get('City', ''), owner.get('ZIP', ''))
                    if not owner.get('ZIP'):
                        owner['ZIP'] = normalize_zip_token(info.get('OwnerZIP', ''))
                    owner['BackfillSource'] = 'FaCTS_Backfill'
                    backfilled = True
                    break
            except Exception:
                continue

        # 2) fallback to stitched property lines
        if not backfilled:
            for ln in property_lines:
                lotsec, _, sp = parse_lotsec_sp(ln)
                if not (lotsec and sp):
                    lotsec, sp = parse_modern_lotsec_sp(ln)
                if not (lotsec and sp):
                    continue
                key = (lotsec, _norm_sp(sp))
                if key in facts_owner_index:
    ...
    if property_lines_for_validation:
        ...
        if facts_sections:
            for ln in property_lines_for_validation:
                ...
                if (not sec) and (facts_space_index is not None):
                    sec2, conf2, method2 = infer_section_from_facts_space(ln, facts_space_index)
                    if sec2:
                        sec, conf, method = sec2, conf2, method2
                if sec:
    ...
    if needs_review:
        # v76: OCR bottom notes band on review pages to capture TRANSFER / DEL / UNSELECTED evidence
        try:
            if pil_page is None:
                pil_page = render_page(pdf_path, page_num - 1, dpi=dpi)
            notes_ocr = ocr_notes_band(pil_page, start_ratio=0.75) if pil_page is not None else ''
            owner['NotesOCR'] = notes_ocr
            u_notes = (notes_ocr or '').upper()
            owner['IsTransferredOutEvidence'] = bool(TRANSFER_RE.search(u_notes)) or owner.get('IsTransferredOutEvidence', False)
            owner['IsDeliveredEvidence'] = bool(DELIVERED_RE.search(u_notes)) or owner.get('IsDeliveredEvidence', False)
            owner['IsUnselectedEvidence'] = bool(UNSELECTED_RE.search(u_notes)) or owner.get('IsUnselectedEvidence', False)
            m_to = re.search(r"TRANSFER(?:RED)?\s+ALL\s+TO\s+([A-Z][A-Z \-\']{2,40})", u_notes)
            owner['TransferToEvidence'] = m_to.group(1).strip() if m_to else owner.get('TransferToEvidence', '')
            m_dt = DATE_RE.search(u_notes)
            owner['TransferDateEvidence'] = m_dt.group(1) if m_dt else owner.get('TransferDateEvidence', '')
        except Exception:
            owner.setdefault('NotesOCR','')
            owner.setdefault('IsTransferredOutEvidence', False)
            owner.setdefault('IsDeliveredEvidence', False)
            owner.setdefault('IsUnselectedEvidence', False)
            owner.setdefault('TransferToEvidence','')
            owner.setdefault('TransferDateEvidence','')
        try:
            if pil_page is None:
                pil_page = render_page(pdf_path, page_num - 1, dpi=dpi)
...
ZIP_OCR_TOKEN_RE = re.compile(r"\b[0-9OIl|SZBs]{5}(?:-[0-9OIl|SZBs]{4})?\b")
def normalize_zip_candidates(line: str) -> str:
    """Repair common OCR confusions in ZIP-like tokens only (e.g., 37O12 -> 37012)."""
    if not line:
        return ''
    def repl(mm):
        return mm.group(0).translate(ZIP_FIX_TABLE)
    return ZIP_OCR_TOKEN_RE.sub(repl, line)


# v83: ZIP / State robustness helpers
# - collapse spaced digit zips: '3 7 2 0 7' -> '37207', '3 7180' -> '37180'
# - drop trailing letters: '37207W' -> '37207'
ZIP_SPACED_RE = re.compile(r"(?:\d\s+){4,}\d")
ZIP_ONE_PLUS_FOUR_RE = re.compile(r"(\d)\s+(\d{4})")
ZIP_TRAIL_LETTER_RE = re.compile(r"(\d{5})[A-Z]")

def normalize_zip_loose(line: str) -> str:
    # tolerant ZIP cleanup (safe to run on any line)
    if not line:
        return ''
    s = str(line)
    try:
        s = normalize_zip_candidates(s)
    except Exception:
        pass
    try:
        s = ZIP_ONE_PLUS_FOUR_RE.sub(lambda m: m.group(1) + m.group(2), s)
    except Exception:
        pass
    def _collapse(mm):
        return re.sub(r"\s+", "", mm.group(0))
    try:
        s = ZIP_SPACED_RE.sub(_collapse, s)
    except Exception:
        pass
    try:
        s = ZIP_TRAIL_LETTER_RE.sub(lambda m: m.group(1), s.upper())
    except Exception:
        pass
    return s

# v83: street normalization (common OCR junk)
LEADING_LIKE_ONE_RE = re.compile(r"^\s*([\/|]*)([Il])\s*(\d)")
LEADING_SPACED_DIGITS_RE = re.compile(r"^\s*((?:\d\s+){1,6}\d)")
...
def normalize_street(street: str) -> str:
    ...
    s = s.replace('\\', ' ').replace('/', ' ')
    s = normalize_ws(s)
    s = LEADING_LIKE_ONE_RE.sub(lambda m: (m.group(1) or '') + '1' + m.group(3), s)
    m2 = LEADING_SPACED_DIGITS_RE.match(s)
    if m2:
        digits = re.sub(r"\s+", "", m2.group(1))
        rest = s[m2.end():].lstrip()
        s = normalize_ws(digits + (' ' + rest if rest else ''))
    return s
...
def heuristic_is_property(line: str) -> bool:
    ...
    u = (line or '').upper()
    # v80: do not mark as property without a strong locator signal
    if not has_strong_property_locator(u):
        return False
...
PROP_FIX_TABLE = str.maketrans({
    'O':'0','I':'1','l':'1','|':'1',
    'S':'5','B':'8',
})
def normalize_prop_token(s: str) -> str:
    """Normalize a property/section token for matching (OCR-safe)."""
    s = (s or '').strip().upper()
    s = s.replace('5EC', 'SEC').replace('5ECTION', 'SECTION')
    s = s.replace('5P', 'SP').replace('5PACE', 'SPACE')
    s = re.sub(r'[^A-Z0-9:/\-\. ]', '', s)
    s = s.translate(PROP_FIX_TABLE)
    s = re.sub(r'\s+', ' ', s)
    return s



# --- v76: property line stitching + FaCTS Space fallback + bottom-notes OCR ---
LOTSEC_RE = re.compile(r"\bLOT\s*[: ]\s*(\d{1,4})\b", re.IGNORECASE)
SEC_RE = re.compile(r"\bSEC(?:TION)?\.?\s*[,:\.]?\s*([A-Z0-9IL]{1,4})\b", re.IGNORECASE)
SP_RE = re.compile(r"\bSP(?:ACE)?\.?\s*[:\.]?\s*([0-9IL]{1,3}(?:\s*[-,]\s*[0-9IL]{1,3})*)\b", re.IGNORECASE)
LOTSECTION_FACTS_RE = re.compile(r"LOT\s*/\s*SECTION\s*([A-Z0-9]+)\s*SPACE\s*([0-9A-Z]+(?:[-,][0-9A-Z]+)*)", re.IGNORECASE)
COORD_3_RE = re.compile(r"\b(\d{1,4})\s*[:/]\s*([A-Z0-9IL]{1,4})\s*[:/]\s*([0-9IL]{1,3}(?:[-,][0-9IL]{1,3})*)\b", re.IGNORECASE)

TRANSFER_RE = re.compile(r"\bTRANSFER(?:RED)?\b", re.IGNORECASE)
DELIVERED_RE = re.compile(r"\bDEL\b|\bDELIVER(?:ED|Y)?\b", re.IGNORECASE)
UNSELECTED_RE = re.compile(r"\bUN\s*-?\s*SELECT(?:ED|ION)?\b|\bUNSELECTED\b", re.IGNORECASE)
DATE_RE = re.compile(r"\b(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})\b")

def _norm_sp_facts(sp: str) -> str:
    """Normalization for FaCTS-originating space tokens (NO OCR swaps)."""
    if not sp:
        return ''
...
def _expand_space_token(sp: str) -> list[str]:
    """
    Expand '1-4' -> ['1','2','3','4'] when numeric.
    Expand '1,3,5' (or normalized '1-3-5') -> ['1','3','5'].
    Preserve alphanumeric tokens as-is.
    """
    if not sp:
        return []
    s = _norm_sp_facts(sp)
...
def _norm_sp(sp: str) -> str:
    if not sp:
        return ''
    s = normalize_prop_token(sp)
    s = re.sub(r"\s+", "", s)
    s = s.replace(',', '-')
    return s

def parse_modern_lotsec_sp(text: str) -> tuple[str, str]:
    """Parse modern two-token coords like 'Everlasting Life 73A 1' (or similar).
    Returns (lotsec, sp) where lotsec looks like '73A' and sp is normalized via _norm_sp.
    """
    if not text:
        return ('', '')
...
def has_strong_property_locator(text: str) -> bool:
    # v83: require strong locator structure (not just garden words)
    if not text:
        return False
    u = normalize_prop_token(str(text)).upper()

    try:
        if RIGHTS_NOTATION_RE.search(u):
            return True
    except Exception:
        pass

    try:
        if COORD_3_RE.search(u) or STRUCT_PROP_RE.search(u) or COLON_LOC_RE.search(u):
            return True
    except Exception:
        pass

    try:
        if LOTSECTION_FACTS_RE.search(u):
            return True
    except Exception:
        pass
    ...
    # Mausoleum / niche: require locator components (C/L, Row, Depth, or explicit Niche code)
    maus_hint = bool(re.search(r"(MAU|MAUS|MAUSOLEUM|NICHE|COLUMBARIUM|C/L|CL|ROW|DPTH|DEPTH|BLDG|BUILDING|COURTSIDE)", u))
    if maus_hint:
        has_code = (
            bool(re.search(r"(C/L|CL)\s*[-:]?\s*[A-Z0-9]{1,4}", u)) or
            bool(re.search(r"ROW\s*[-:]?\s*[A-Z0-9]{1,4}", u)) or
            bool(re.search(r"(DPTH|DEPTH)\s*[-:]?\s*\d{1,3}", u)) or
            bool(re.search(r"NICHE\s*[A-Z]\s*\d{1,3}", u)) or
            bool(re.search(r"NICHE\s*[A-Z]\d{1,3}", u))
        )
        if has_code:
            return True
...
def parse_lotsec_sp(text: str) -> tuple[str, str, str]:
    """Extract (lotsec, sec_only, sp) from a property-like string."""
    if not text:
        return ('', '', '')
    u = normalize_prop_token(text)
    m3 = COORD_3_RE.search(u)
    if m3:
        lot = m3.group(1)
        sec = m3.group(2)
        sp = _norm_sp(m3.group(3))
        return (f"{lot}{sec}", sec, sp)
    lot_m = LOTSEC_RE.search(u)
    sec_m = SEC_RE.search(u)
    sp_m = SP_RE.search(u)
    lot = lot_m.group(1) if lot_m else ''
    sec = sec_m.group(1) if sec_m else ''
    sp = _norm_sp(sp_m.group(1)) if sp_m else ''
    lotsec = f"{lot}{sec}" if lot and sec else ''
    return (lotsec, sec, sp)

def stitch_property_lines(lines_in: list[str]) -> list[str]:
    """Merge adjacent property fragments (e.g., 'Sec C Sp 1-2' above 'Lot 117')."""
    if not lines_in:
        return []
    out = []
    i = 0
    while i < len(lines_in):
        a = lines_in[i] or ''
        ua = a.upper()
        a_has_lot = 'LOT' in ua or bool(re.search(r"\bLO[T]\b", ua))
        a_has_secsp = bool(re.search(r"\bSEC\b|\bSECTION\b", ua)) or bool(re.search(r"\bSP\b|\bSPACE\b", ua))
        a = re.sub(r"\bLO[T]\b", "LOT", a, flags=re.IGNORECASE)
        if i + 1 < len(lines_in):
            b = lines_in[i + 1] or ''
            ub = b.upper()
            b_has_lot = 'LOT' in ub or bool(re.search(r"\bLO[T]\b", ub))
            b_has_secsp = bool(re.search(r"\bSEC\b|\bSECTION\b", ub)) or bool(re.search(r"\bSP\b|\bSPACE\b", ub))
            b = re.sub(r"\bLO[T]\b", "LOT", b, flags=re.IGNORECASE)
            if (a_has_secsp and not a_has_lot and b_has_lot) or (a_has_lot and not a_has_secsp and b_has_secsp):
                out.append(normalize_ws(f"{a} {b}"))
                i += 2
                continue
...
def read_facts_owner_details_df(facts_path: str) -> pd.DataFrame:
    """Read the 'Owner Details' export as a dataframe with header auto-detect."""
    if not facts_path or not os.path.exists(facts_path):
        return pd.DataFrame()
    try:
        preview = pd.read_excel(facts_path, engine='openpyxl', sheet_name=0, header=None, nrows=30)
...
def build_facts_space_index(facts_df: pd.DataFrame) -> dict:
    """Map (lotsec, sp) -> set(sections) using FaCTS Space column text.

    v79:
      - Supports alphanumeric and range/comma space tokens.
      - Adds expanded numeric space keys (e.g., 1-4 -> 1,2,3,4) to improve matching.
    """
    if facts_df is None or facts_df.empty:
        return {}

    cols = {str(c).strip().lower(): c for c in facts_df.columns}
    sec_col = cols.get('section') or next((v for k, v in cols.items() if 'section' in k), None)
    space_col = cols.get('space') or next((v for k, v in cols.items() if k == 'space' or ' space' in k), None)
    if sec_col is None or space_col is None:
        return {}

    idx = {}
    df2 = facts_df[[sec_col, space_col]].fillna('')
    for row in df2.itertuples(index=False, name=None):
        sec = str(row[0]).strip()
        sptext = str(row[1]).strip()
        if not sec or not sptext:
            continue
        u = normalize_prop_token(sptext)
        m = LOTSECTION_FACTS_RE.search(u)
        if m:
            lotsec = normalize_prop_token(m.group(1)).replace(' ', '')
            sp_raw = m.group(2)
        else:
            lotsec, _, sp_raw = parse_lotsec_sp(u)
        if not lotsec or not sp_raw:
            continue
        tokens = [sp_raw] + _expand_space_token(sp_raw)
        for tok in tokens:
            sp = _norm_sp(tok)
            if sp:
                idx.setdefault((lotsec, sp), set()).add(sec)
    return idx


# v83: Mausoleum / niche FaCTS matching support (C/L / Row / Depth / Niche)
MAUSLIKE_HINT_RE = re.compile(r"(MAU|MAUS|MAUSOLEUM|NICHE|COLUMBARIUM|C/L|CL|ROW|DPTH|DEPTH|COURTSIDE|BLDG|BUILDING)", re.IGNORECASE)

def normalize_maus_text(s: str) -> str:
    if not s:
        return ''
    u = normalize_ws(str(s).upper())
    u = u.replace('C/L', 'CL')
    u = re.sub(r"C\s+L", "CL", u)
    u = re.sub(r"DEPTH", "DPTH", u)
    u = re.sub(r"DPT", "DPTH", u)
    u = re.sub(r"[^A-Z0-9\-:/ ]", " ", u)
    u = re.sub(r"\s+", " ", u).strip()
    return u

def maus_component_key(s: str) -> str:
    u = normalize_maus_text(s)
    if not u or not MAUSLIKE_HINT_RE.search(u):
        return ''

    base = ''
    m = re.search(r"MAU\s*[- ]\s*([A-Z0-9]{1,16})", u)
    if m:
        base = f"MAU-{m.group(1)}"
    else:
        m2 = re.search(r"([A-Z0-9 ]{3,60}?)\s+NICHE", u)
        if m2:
            base = (m2.group(1).strip() + " NICHE").strip()
        else:
            m3 = re.search(r"([A-Z0-9 ]{3,60}?)\s+(MAUSOLEUM|MAUS|COURTSIDE|COLUMBARIUM|BLDG|BUILDING)", u)
            if m3:
                base = (m3.group(1).strip() + " " + m3.group(2)).strip()
    ...
    mrow = re.search(r"ROW\s*[-:]?\s*([A-Z0-9]{1,4})", u)
    if mrow:
        row = mrow.group(1)
    mcl = re.search(r"CL\s*[-:]?\s*([A-Z0-9]{1,4})", u)
    if mcl:
        cl = mcl.group(1)
    md = re.search(r"DPTH\s*[-:]?\s*(\d{1,3})", u)
    if md:
        dpth = md.group(1)
    mn = re.search(r"NICHE\s*([A-Z])\s*(\d{1,3})", u)
    if mn:
        niche = f"{mn.group(1)}{mn.group(2)}"
    else:
        mn2 = re.search(r"NICHE\s*([A-Z]\d{1,3})", u)
        if mn2:
            niche = mn2.group(1)

    parts = []
    if base: parts.append(base)
    if row: parts.append(f"ROW={row}")
    if cl: parts.append(f"CL={cl}")
    if dpth: parts.append(f"DPTH={dpth}")
    if niche: parts.append(f"NICHE={niche}")
    return '|'.join(parts) if parts else ''

def build_facts_maus_index(facts_df: pd.DataFrame) -> dict:
    # Index maus/niche style Space rows from FaCTS: key -> list of entries
    if facts_df is None or facts_df.empty:
        return {}
    cols = {str(c).strip().lower(): c for c in facts_df.columns}
    sec_col = cols.get('section') or next((v for k, v in cols.items() if 'section' in k), None)
    ...
    use_cols = [c for c in [sec_col, space_col, owner_col, addr_col, city_col, st_col, zip_col] if c is not None]
    df2 = facts_df[use_cols].fillna('')
    pos = {c:i for i,c in enumerate(use_cols)}
    p_sec = pos.get(sec_col)
    p_space = pos.get(space_col)

    idx = {}
    for row_i, row in enumerate(df2.itertuples(index=False, name=None), start=1):
        sec = str(row[p_sec] or '').strip()
        sptext = str(row[p_space] or '').strip()
        if not sec or not sptext:
            continue
        if not MAUSLIKE_HINT_RE.search(sptext):
            continue
        key = maus_component_key(sptext) or normalize_maus_text(sptext)
        if not key:
            continue
        def _get(col):
            return str(row[pos[col]]).strip() if (col is not None and col in pos) else ''
        entry = {
            'Section': sec,
            'Space': sptext,
            'RowIndex': row_i,
            'OwnerName': _get(owner_col),
            'OwnerAddress': _get(addr_col),
            'OwnerCity': _get(city_col),
            'OwnerState': _get(st_col),
            'OwnerZIP': normalize_zip_token(_get(zip_col)) if zip_col is not None else '',
        }
        idx.setdefault(key, []).append(entry)
        nf = normalize_maus_text(sptext)
        if nf and nf != key:
            idx.setdefault(nf, []).append(entry)
    return idx

def infer_section_from_facts_space(line_text: str, facts_space_index: dict) -> tuple[str, float, str]:
    if not line_text or not facts_space_index:
        return ('', 0.0, '')
    lotsec, _, sp = parse_lotsec_sp(line_text)
    if not lotsec or not sp:
        return ('', 0.0, '')
    cands = list(facts_space_index.get((lotsec, sp), set()))
    if len(cands) == 1:
        return (cands[0], 0.90, 'facts_space_unique')
    return ('', 0.0, '')

def ocr_notes_band(pil_page: Image.Image, start_ratio: float = 0.75) -> str:
    """OCR bottom notes band for handwritten TRANSFER/DEL/UNSELECTED cues."""
    if pil_page is None:
        return ''
    try:
        w, h = pil_page.size
        crop = pil_page.crop((0, int(h * start_ratio), w, h))
        variants = [preprocess_shadow_correct(crop), preprocess_clahe(crop), unsharp_mask(crop)]
        best = ''
        best_sc = -1
        for img in variants:
            for psm in (6, 11):
                t = _tess_image_to_string(img, config=f"{TESS_CONFIG_BASE} --psm {psm}")
                u = (t or '').upper()
                sc = 0
                if TRANSFER_RE.search(u): sc += 50
                if UNSELECTED_RE.search(u): sc += 40
                if DELIVERED_RE.search(u): sc += 30
                sc += min(len([ln for ln in split_lines(t) if ln and not is_gibberish(ln)]), 25)
                if sc > best_sc:
                    best_sc = sc
                    best = t
        return best or ''
        ...
        return ''
def extract_zip_state(text: str) -> Tuple[str, str, str]:
    """Extract City, State, ZIP from a single line."""
    rz = rescue_zip(text or '')
    line2 = fix_state_ocr_tokens(text or '')
    line2 = normalize_zip_candidates(line2)
    line2 = normalize_zip_loose(line2)
    zipm = re.search(ZIP_RE, line2)
    statem = find_state_match(line2, zipm)
    zip5 = zipm.group(0)[:5] if zipm else ''
    state = normalize_state(statem.group(0)) if statem else ''

    city = ''
    if statem:
        city = line2[:statem.start()].strip(" ,")
    elif zipm:
        city = line2[:zipm.start()].strip(" ,")
    city = re.sub(ZIP_RE, '', city).strip(" ,")

    if not zip5:
        zip5 = rz
    state = normalize_state_token(state, city, zip5)
    return city, state, zip5

def looks_like_address_line(line: str) -> bool:
    if not line:
        return False
    _, st, z = extract_zip_state(line)
    if z and st:
        return True
...
def parse_best_address(lines: List[str]) -> Dict:
    ...
    return {'Index': best.get('Index'), 'Street': normalize_street(street), 'City': city, 'State': state, 'ZIP': zipc, 'AddressRaw': address_raw}

def extract_state_zip_anywhere(text: str) -> Tuple[str, str]:
    """Best-effort: find a ZIP and nearby state anywhere in text.
    ...
    t_norm = normalize_zip_loose(t_norm)
...
def parse_owner_header(lines: List[str], target_char: Optional[str] = None) -> Tuple[str, str, str, str, Dict, bool]:
    ...
    primary = fix_target_letter(primary, target_char or '')
...
def item_dict_from_line(txt: str, struck: bool = False) -> Dict:
    ...
    return {
        ...
        "Prop_Type": "",
        "Prop_RawKey": "",
        "FaCTS_MatchConfidence": "",
        "FaCTS_MatchMethod": "",
        "FaCTS_MatchedSpace": "",
        "FaCTS_MatchedSections": "",
 "FaCTS_MatchedRowIndex": "",
    }
...
# v83: Hopeless-page header rescue (3-strike rule)

def should_rescue_header(primary_name: str, street: str, state: str, zip5: str, raw_text: str) -> bool:
    flags = 0
    pn = str(primary_name or '')
    if (not pn.strip()) or ('[MISSING' in pn.upper()) or is_gibberish(pn):
        flags += 1
    z = str(zip5 or '')
    if not re.fullmatch(r"\d{5}", z.strip()):
        flags += 1
    st = str(street or '').strip()
    if (not st) or ((not re.search(STREET_START_RE, st)) and (not re.search(r"PO\s*BOX", st, re.IGNORECASE))):
        flags += 1
    rt = str(raw_text or '')
    if score_text_pass(rt) < 18:
        flags += 1
    return flags >= 2

def rescue_header_high_dpi(pdf_path: str, page_index: int, dpi_base: int, target_char: Optional[str],
                           kraken_model: str = '', kraken_bin: str = 'kraken', kraken_python: str = '',
                           allow_livetext: bool = True, alt_ocr: str = 'ocrmac_then_kraken') -> tuple[str, dict, dict]:
    dpi_rescue = int(max(400, min(450, max(dpi_base, 400))))
    pil = render_page_deskew(pdf_path, page_index, dpi=dpi_rescue)
    if pil is None:
        return '', {'used': False, 'dpi': dpi_rescue}, {}
    htxt, hmeta = ocr_header_ensemble(pil, kraken_model=kraken_model, kraken_bin=kraken_bin,
                                     kraken_python=kraken_python, allow_livetext=allow_livetext, alt_ocr=alt_ocr)
    lines = split_lines(htxt)
    _, p, s, last, addr, _ = parse_owner_header(lines, target_char)
    addr = dict(addr or {})
    addr['Street'] = normalize_street(addr.get('Street',''))
    if addr.get('CityStateZip'):
        addr['CityStateZip'] = normalize_zip_loose(addr['CityStateZip'])
    if addr.get('ZIP'):
        addr['ZIP'] = normalize_zip_token(addr.get('ZIP'))
    if addr.get('State'):
        addr['State'] = normalize_state_token(addr.get('State',''), addr.get('City',''), addr.get('ZIP',''))
    hmeta = dict(hmeta or {})
    hmeta['used'] = True
    hmeta['dpi'] = dpi_rescue
    return (p, hmeta, addr)

def process_page(pdf_path: str, page_index: int, dpi: int, target_char: Optional[str], reader: Optional[PdfReader] = None, kraken_model: str = '', kraken_bin: str = 'kraken', kraken_python: str = '', allow_livetext: bool = True, facts_sections: Optional[List[str]] = None, facts_mode: str = '', facts_space_index: Optional[dict] = None, facts_owner_index: Optional[dict] = None, facts_maus_index: Optional[dict] = None, alt_ocr: str = "ocrmac_then_kraken") -> Tuple[Dict, List[Dict], bool]:
...
owner = enrich_owner_with_review(owner, items, pdf_path, page_index+1, dpi=dpi, pil_page=pil_original, facts_sections=facts_sections, facts_mode=facts_mode, reader=reader, facts_space_index=facts_space_index, facts_owner_index=facts_owner_index, facts_maus_index=facts_maus_index)
    if (not owner.get('ZIP')) or (not owner.get('State')) or ('[MISSING' in str(owner.get('PrimaryOwnerName','')).upper()):
        ...
        if (not owner.get('PrimaryOwnerName')) or ('[MISSING' in str(owner.get('PrimaryOwnerName','')).upper()):
            if parsed.get('PrimaryOwnerName'):
                owner['PrimaryOwnerName'] = parsed.get('PrimaryOwnerName', '')
        if not owner.get('Street'):
            owner['Street'] = parsed.get('Street', '') or owner.get('Street', '')
        if not owner.get('City'):
            owner['City'] = parsed.get('City', '') or owner.get('City', '')
        if not owner.get('ZIP'):
            owner['ZIP'] = parsed.get('ZIP', '') or owner.get('ZIP', '')
        if not owner.get('State'):
            owner['State'] = normalize_state_token(parsed.get('State', ''), owner.get('City', ''), owner.get('ZIP', ''))
        if not owner.get('AddressRaw') and parsed.get('AddressRaw'):
            owner['AddressRaw'] = parsed.get('AddressRaw', '')
    return owner, items, is_interment
...
def process_dataset(pdf_path: str, out_path: str, dpi: int = 300, kraken_model: str = '', kraken_bin: str = 'kraken', kraken_python: str = '', allow_livetext: bool = True, facts_path: str = '', alt_ocr: str = "ocrmac_then_kraken"):
    ...
    if facts_path:
        if not os.path.exists(facts_path):
            ...
            facts_df = read_facts_owner_details_df(facts_path)
            facts_space_index = build_facts_space_index(facts_df)
            facts_owner_index = build_facts_owner_index(facts_df)
            facts_maus_index = build_facts_maus_index(facts_df)
    ...
    for p in tqdm(range(page_count), desc=f"Scanning {filename}", unit="page"):
        owner_data, items_data, is_interment = process_page(
            ...
            facts_space_index=facts_space_index,
            facts_owner_index=facts_owner_index,
            facts_maus_index=facts_maus_index,
            alt_ocr=alt_ocr,
        )
        ...
        if is_interment:
            ...
            is_card, reason, sc = page_is_card(owner_data, items_data, owner_data.get('RawText', ''))
            owner_data['CardScore'] = sc
            if not is_card:
                skipped_rows.append({
                    'SourceFile': filename,
                    'PageNumber': p + 1,
                    'SkipReason': reason,
                    'CardScore': sc,
                    'RawText': (owner_data.get('RawText', '') or '')[:300],
                })
            else:
                if sc < 4:
                    owner_data['NeedsReview'] = True
                    note = f'LOW_CONFIDENCE_{sc}'
                    existing = owner_data.get('NeedsReviewNotes', '') or ''
                    owner_data['NeedsReviewNotes'] = (existing + ' | ' + note).strip(' |') if existing else note
    ...
    if not owners_master.empty:
        ...
        # v76: actionable lists from notes OCR evidence
        list_unselected = owners_master[(owners_master.get('IsUnselectedEvidence', False) == True) & (owners_master.get('IsTransferredOutEvidence', False) != True)].copy()
        list_transferred = owners_master[(owners_master.get('IsTransferredOutEvidence', False) == True)].copy()
        list_delivered = owners_master[(owners_master.get('IsDeliveredEvidence', False) == True)].copy()
    else:
        list_memorial = list_pn = list_prime = list_survivor = pd.DataFrame()
        list_unselected = list_transferred = list_delivered = pd.DataFrame()
    ...
    stats = pd.DataFrame([{
        ...
        'LIST_Unselected_Active': int(len(list_unselected)) if 'list_unselected' in locals() and not list_unselected.empty else 0,
        'LIST_TransferredOut_Evidence': int(len(list_transferred)) if 'list_transferred' in locals() and not list_transferred.empty else 0,
        'LIST_Delivered_Evidence': int(len(list_delivered)) if 'list_delivered' in locals() and not list_delivered.empty else 0,
    ...
    list_unselected_safe = make_df_excel_safe(list_unselected) if 'list_unselected' in locals() and not list_unselected.empty else pd.DataFrame()
    list_transferred_safe = make_df_excel_safe(list_transferred) if 'list_transferred' in locals() and not list_transferred.empty else pd.DataFrame()
    list_delivered_safe = make_df_excel_safe(list_delivered) if 'list_delivered' in locals() and not list_delivered.empty else pd.DataFrame()
    ...
    # v83: QC flags sheet (one row per OwnerRecordID)
    qc_df = pd.DataFrame()
    try:
        if not owners_master.empty and 'OwnerRecordID' in owners_master.columns:
            def _flag_missing_name(x):
                s = '' if x is None else str(x)
                return (not s.strip()) or ('[MISSING' in s.upper()) or is_gibberish(s)
            def _flag_missing_zip(x):
                z = '' if x is None else str(x)
                return not bool(re.fullmatch(r"\d{5}", z.strip()))
            def _flag_missing_street(x):
                s = '' if x is None else str(x)
                s = s.strip()
                if not s:
                    return True
                if re.search(r"\bPO\s*BOX\b", s, re.IGNORECASE):
                    return False
                return not bool(re.search(STREET_START_RE, s))
            qc_df = pd.DataFrame({
                'OwnerRecordID': owners_master['OwnerRecordID'],
                'SourceFile': owners_master.get('SourceFile',''),
                'PageNumber': owners_master.get('PageNumber',''),
                'Flag_MissingName': owners_master.get('PrimaryOwnerName','').apply(_flag_missing_name),
                'Flag_MissingStreet': owners_master.get('Street','').apply(_flag_missing_street),
                'Flag_MissingZip': owners_master.get('ZIP','').apply(_flag_missing_zip),
                'Flag_GibberishOCR': owners_master.get('RawText','').apply(lambda t: score_text_pass(str(t or '')) < 18),
                'Flag_NoPropertyLines': owners_master.get('HasPropertyAny', False).apply(lambda v: (v is False) or (str(v).strip().upper() in ('FALSE','0','NO'))),
                'OCR_StrategyUsed': owners_master.get('TextSource','').astype(str) + ' | ' + owners_master.get('HeaderOCRBest','').astype(str) + ' | Rescue=' + owners_master.get('OCRRescueUsed', False).astype(str) + ' DPI=' + owners_master.get('OCRRescueDPI','').astype(str),
                'NeedsManualReview': owners_master.get('NeedsReview', False).fillna(False).astype(bool),
                'NeedsReviewNotes': owners_master.get('NeedsReviewNotes','').astype(str),
                'CardScore': owners_master.get('CardScore',''),
            })
    except Exception:
        qc_df = pd.DataFrame()
    ...
    skipped_safe = make_df_excel_safe(skipped_df) if not skipped_df.empty else pd.DataFrame()
    ...
    with pd.ExcelWriter(tmp_path, engine=engine) as xw:
        ...
        if not skipped_safe.empty:
            skipped_safe.to_excel(xw, index=False, sheet_name="SkippedPages")
        ...
        if 'list_unselected_safe' in locals() and not list_unselected_safe.empty:
            list_unselected_safe.to_excel(xw, index=False, sheet_name='LIST_Unselected_Active')
        if 'list_transferred_safe' in locals() and not list_transferred_safe.empty:
            list_transferred_safe.to_excel(xw, index=False, sheet_name='LIST_TransferredOut_Evidence')
        if 'list_delivered_safe' in locals() and not list_delivered_safe.empty:
            list_delivered_safe.to_excel(xw, index=False, sheet_name='LIST_Delivered_Evidence')
        ...
        if 'qc_df' in locals() and isinstance(qc_df, pd.DataFrame) and not qc_df.empty:
            make_df_excel_safe(qc_df).to_excel(xw, index=False, sheet_name='QC_Flags')
