#!/usr/bin/env python3
"""Owner Card Pipeline — v70.7 (FaCTS Section Validation + Full Snapshots + NeedsReviewNotes)
...
Run:
  python3 owner_cards_pipeline_v70_7.py
  or
  python3 owner_cards_pipeline_v70_7.py --pdf "B (all).pdf" --out "OwnerCards_B_Output.xlsx"
...
# --- Snapshot location: keep with script/batch folder (v70.7) ---
...
def enrich_owner_with_review(owner: Dict, items: List[Dict], pdf_path: str, page_num: int, dpi: int, pil_page: Optional[Image.Image] = None, facts_sections: Optional[List[str]] = None, facts_mode: str = '', reader: Optional[PdfReader] = None) -> Dict:
    ...
    # v70.7: Only validate FaCTS sections on lines that actually look like they contain garden/lot/sec/space patterns.
...
# -----------------------------
# v70.7 PROPERTY HEURISTICS (garden + coordinate OCR fuzz)
# -----------------------------
DEFAULT_GARDENS = [
    "GOOD SHEPHERD",
    "EVERLASTING LIFE",
...
def parse_best_address(lines: List[str]) -> Dict:
    ...
    # v70.7: Loose address hunter - City + State without ZIP
    if not candidates:
...
def clean_name_line(line: str, target_char: Optional[str] = None, aggressive: bool = False) -> str:
    if not line:
        return ""
    # v70.7: Noise Trimmer - cut off clusters of punctuation/symbols (scribbles/dirt)
...
# --- Structural / colon-style property detection (v70.7 fix) ---
    ...
    for tag, img_v in variants:
        ...
        for psm in psm_list:
            ...
            if sc > best_score:
                ...
                # v70.7: stop early when header OCR is already very strong
                if best_score >= 95:
                    break
...
def process_page(pdf_path: str, page_index: int, dpi: int, target_char: Optional[str], reader: Optional[PdfReader] = None, kraken_model: str = '', kraken_bin: str = 'kraken', kraken_python: str = '', allow_livetext: bool = True, facts_sections: Optional[List[str]] = None, facts_mode: str = '', alt_ocr: str = "ocrmac_then_kraken") -> Tuple[Dict, List[Dict], bool]:
    ...
    if text_layer_usable(pdf_text):
        ...
        if ok:
            ...
            # v70.7: salvage State/ZIP from anywhere on page when text-layer address parse is incomplete
            try:
...
def process_dataset(pdf_path: str, out_path: str, dpi: int = 300, kraken_model: str = '', kraken_bin: str = 'kraken', kraken_python: str = '', allow_livetext: bool = True, facts_path: str = '', alt_ocr: str = "ocrmac_then_kraken"):
    ...
    # v70.7 debug: capture lines that look like property candidates but were not classified as property
    debug_rows = []
