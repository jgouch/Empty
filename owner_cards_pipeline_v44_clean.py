#!/usr/bin/env python3
"""Owner Card Pipeline — v44 CLEAN (Trust‑But‑Verify + PhoneRaw/Normalized + PhoneExceptions)

This file is generated to be *paste/run ready* (no HTML entities like &gt;).

Key features:
- Text-layer-first (PyPDF2) when usable
- TRUST-BUT-VERIFY (v43 behavior): if extracted PrimaryOwnerName doesn't start with target letter, force OCR fallback
- Middle-initial rescue: '8.' -> 'B.'
- OCR line dedupe uses text + binned bbox
- Phone fields: PhoneRaw, PhoneNormalized, PhoneAltNormalized + flags (no fabricated area code)
- Excel-safe output + PhoneExceptions sheet
...
Run:
  python3 owner_cards_pipeline_v44_clean.py
  or
  python3 owner_cards_pipeline_v44_clean.py --pdf "B (all).pdf" --out "OwnerCards_B_Output.xlsx"
"""
...
# -----------------------------
# PAGE PROCESSOR (Trust-But-Verify + Phones)
# -----------------------------
...
def process_page(pdf_path: str, page_index: int, dpi: int, target_char: Optional[str], reader: Optional[PdfReader] = None) -> Tuple[Dict, List[Dict], bool]:
    ...
    if text_layer_usable(pdf_text):
        ...
        # trust-but-verify: require target letter when present
