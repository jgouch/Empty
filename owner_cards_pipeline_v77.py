#!/usr/bin/env python3
"""Owner Card Pipeline — v77 (Line Stitching + FaCTS Space Fallback + Notes OCR Lists)
...
Run:
  python3 owner_cards_pipeline_v77.py
  or
  python3 owner_cards_pipeline_v77.py --pdf "B (all).pdf" --out "OwnerCards_B_Output.xlsx"
...
def normalize_state_token(state: str, city: str = '', zip5: str = '') -> str:
    ...
    if not state:
        return ''
    st = re.sub(r"[^A-Z]", "", str(state).upper())
    if st == 'IN':
        cz = str(zip5 or '')
...
def build_facts_owner_index(facts_df: pd.DataFrame) -> dict:
    ...
    out = {}
    for _, row in facts_df[use_cols].fillna('').iterrows():
        sptext = str(row.get(space_col,'') or '').strip()
        if not sptext:
            continue
        lotsec, _, sp = parse_lotsec_sp(sptext)
        if not lotsec or not sp:
            continue
        key = (lotsec, _norm_sp(sp))
        if key not in out:
            out[key] = {
                'OwnerName': str(row.get(owner_col,'') or '').strip() if owner_col else '',
                'OwnerAddress': str(row.get(addr_col,'') or '').strip() if addr_col else '',
                'OwnerCity': str(row.get(city_col,'') or '').strip() if city_col else '',
                'OwnerState': str(row.get(st_col,'') or '').strip() if st_col else '',
                'OwnerZIP': normalize_zip_token(str(row.get(zip_col,'') or '')) if zip_col else '',
            }
    return out
...
def load_facts_sections(facts_path: str) -> List[str]:
    ...
    try:
        ...
        for i in range(min(len(preview), 30)):
            ...
            if 'section' in vals and 'space' in vals:
                header_row = i
                break
...
LOTSECTION_FACTS_RE = re.compile(r"LOT\s*/\s*SECTION\s*([A-Z0-9]+)\s*SPACE\s*([0-9]+(?:[-,][0-9]+)*)", re.IGNORECASE)
...
def build_facts_space_index(facts_df: pd.DataFrame) -> dict:
    ...
    if sec_col is None or space_col is None:
        return {}
    idx = {}
    for _, row in facts_df[[sec_col, space_col]].dropna().iterrows():
        sec = str(row[sec_col]).strip()
        sptext = str(row[space_col]).strip()
        ...
        if m:
            ...
            sp = _norm_sp(m.group(2))
        else:
            lotsec, _, sp = parse_lotsec_sp(u)
        if not lotsec or not sp:
            continue
