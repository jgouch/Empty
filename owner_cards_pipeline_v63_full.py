#!/usr/bin/env python3
"""Owner Card Pipeline — v63 (FaCTS Section Validation + Full Snapshots + NeedsReviewNotes)
...
def enrich_owner_with_review(owner: Dict, items: List[Dict], pdf_path: str, page_num: int, dpi: int, pil_page: Optional[Image.Image] = None, facts_sections: Optional[List[str]] = None) -> Dict:
...
def _auto_rotate_by_header_score(pil_img: Image.Image, header_ratio: float = 0.35) -> tuple[Image.Image, dict]:
    ...
    for angle in (0, 90, 180, 270):
        try:
            ...
            if sc > best_meta.get('score', -1):
                best = cand
                best_meta = {"angle": angle, "score": sc}
        except Exception:
            continue
...
def process_page(pdf_path: str, page_index: int, dpi: int, target_char: Optional[str], reader: Optional[PdfReader] = None, kraken_model: str = '', kraken_bin: str = 'kraken', kraken_python: str = '', allow_livetext: bool = True, facts_sections: Optional[List[str]] = None, alt_ocr: str = "ocrmac_then_kraken") -> Tuple[Dict, List[Dict], bool]:
    ...
    if text_layer_usable(pdf_text):
        ...
        if ok:
            ...
            owner = enrich_owner_with_review(owner, items, pdf_path, page_index+1, dpi=dpi, pil_page=None, facts_sections=facts_sections)
    ...
    owner.update(phone_fields)
    owner = enrich_owner_with_review(owner, items, pdf_path, page_index+1, dpi=dpi, pil_page=pil_original, facts_sections=facts_sections)
    return owner, items, is_interment
