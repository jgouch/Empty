#!/usr/bin/env python3
"""Owner Card Pipeline — v70.1 (FaCTS Section Validation + Full Snapshots + NeedsReviewNotes)
...
Run:
  python3 owner_cards_pipeline_v70_1.py
  or
  python3 owner_cards_pipeline_v70_1.py --pdf "B (all).pdf" --out "OwnerCards_B_Output.xlsx"
...
def save_failure_snapshots(pil_img: Image.Image, source_pdf: str, page_num: int, reason: str = "NEEDS_REVIEW") -> Dict[str, str]:
    ...
    # Auto-rotate BEFORE cropping
    rot_img, meta = _auto_rotate_by_header_score(pil_img, header_ratio=0.35)
...
# -----------------------------
# v70.1 PROPERTY HEURISTICS (garden + coordinate OCR fuzz)
# -----------------------------
DEFAULT_GARDENS = [
    "GOOD SHEPHERD",
    "EVERLASTING LIFE",
...
def heuristic_is_property(line: str) -> bool:
    if not line:
        return False
    if not RE_GARDEN_CHECK.search(line):
        return False
    if COORD_OCR_FUZZ_RE.search(line):
        return True
    toks = re.findall(r"[A-Z0-9]+", line.upper())
    score = 0
    for t in toks:
