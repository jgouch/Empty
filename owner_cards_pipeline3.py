...
#!/usr/bin/env python3
"""
Owner Card Pipeline (Local) — Excel-Safe + Excel-Length-Safe (Final Hardened v44)
...
V44 Improvements:
- TRUST BUT VERIFY (from your v43): If the Text Layer (PyPDF2) fails to yield a valid name
  starting with Target Letter, we reject text layer and force OCR fallback.
- Adds PhoneRaw + PhoneNormalized + PhoneAltNormalized + validation flags (no fabricated 615).
- Adds "PhoneExceptions" sheet for quick cleanup during file review.
- Keeps bbox+text dedupe for line items (prevents dropping legit duplicates).
"""
...
STATE_MAP = {
    ...
    # TN typos only (keep conservative)
    "TENN": "TN", "TENNESSES": "TN", "TEN": "TN", "TENN.": "TN", "TN.": "TN",
    "TIN": "TN",
    # Explicitly keep IN as IN (no IN->TN bug)
    "IN": "IN",
}
...
def extract_phone_fields(full_text: str, lines: List[str]) -> Dict[str, object]:
    ...
    return {
        "PhoneRaw": phone_raw,
        "Phone": primary[1] if primary[1] else "",              # legacy convenience column
...
# -----------------------------
# INITIAL PRESERVATION + HEADER PARSING
# -----------------------------
...
def repair_word_start(word: str, target_char: str) -> str:
    """Your v43 prefix surgeon style repair."""
...
def clean_name_line(line: str, target_char: Optional[str] = None, aggressive: bool = False) -> str:
    ...
    m_kw = re.search(r"\b(road|rd|street|st|avenue|ave|drive|dr|lane|ln|court|ct|blvd|boulevard|pkwy|parkway|hwy|highway|trl|trail|cir|circle|pl|place|po\s*box|box)\b",
                     line, re.IGNORECASE)
    if m_kw:
...
def parse_owner_header(lines: List[str], target_char: Optional[str] = None) -> Tuple[str, str, str, str, Dict, bool]:
    ...
    # final tighten: if target exists and still doesn't match, label missing
    primary = ""
    secondary = ""
...
# -----------------------------
# ITEM CLASSIFICATION / EXTRACTION
# -----------------------------
...
def item_dict_from_line(txt: str, struck: bool = False) -> Dict:
    ...
    looks_item = (
        cls["IsProperty"] or cls["IsMemorial"] or cls["IsFuneralPreneed"]
        or cls["IsAtNeedFuneral"] or cls["HasRightsNotation"]
    )
...
# -----------------------------
# SCORING (OCR fallback selection)
# -----------------------------
...
# -----------------------------
# PAGE PROCESSOR (V43 TRUST-BUT-VERIFY + V44 phones)
# -----------------------------
...
def process_page(pdf_path: str, page_index: int, dpi: int, target_char: Optional[str], reader: Optional[PdfReader] = None) -> Tuple[Dict, List[Dict], bool]:
    ...
    txt = ""
    lines = []
    template_type = "legacy_typewritten"
    is_interment = False
    p = s = last = ""
    addr = {}
    ...
    def add_lines(lines_ocr):
        for ln_obj in lines_ocr:
            ...
            # bbox+text dedupe so repeated lines survive
    ...
    owner.update(phone_fields)
    return owner, all_items, is_interment
...
def process_dataset(pdf_path: str, out_path: str, dpi: int = 300):
    ...
    try:
        ...
        # fallback to PyPDF2 before rendering thumbs
        try:
            page_count = len(PdfReader(pdf_path).pages)
            print(f"[Info] PyPDF2 page count: {page_count}")
        except Exception:
            print("[Warn] page count failed, falling back to render...")
    ...
    # Possible duplicates by RawTextHash
