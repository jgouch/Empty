#!/usr/bin/env python3
"""Owner Card Pipeline — v76 (Line Stitching + FaCTS Space Fallback + Notes OCR Lists)
...
Run:
  python3 owner_cards_pipeline_v76.py
  or
  python3 owner_cards_pipeline_v76.py --pdf "B (all).pdf" --out "OwnerCards_B_Output.xlsx"
...
def enrich_owner_with_review(owner: Dict, items: List[Dict], pdf_path: str, page_num: int, dpi: int, pil_page: Optional[Image.Image] = None, facts_sections: Optional[List[str]] = None, facts_mode: str = '', reader: Optional[PdfReader] = None, facts_space_index: Optional[dict] = None) -> Dict:
    ...
    if needs_review:
        ...
        try:
            ...
            owner['IsTransferredOutEvidence'] = bool(TRANSFER_RE.search(u_notes))
            owner['IsDeliveredEvidence'] = bool(DELIVERED_RE.search(u_notes))
            owner['IsUnselectedEvidence'] = bool(UNSELECTED_RE.search(u_notes))
            m_to = re.search(r"TRANSFER(?:RED)?\s+ALL\s+TO\s+([A-Z][A-Z \-\']{2,40})", u_notes)
            owner['TransferToEvidence'] = m_to.group(1).strip() if m_to else ''
            m_dt = DATE_RE.search(u_notes)
            owner['TransferDateEvidence'] = m_dt.group(1) if m_dt else ''
        except Exception:
...
def extract_zip_state(line: str) -> Tuple[Optional[str], Optional[str]]:
    """Extract ZIP and State from a single line.

    v75: applies ZIP OCR rescue ONLY to ZIP-like tokens before regex detection.
    """
...
def process_page(pdf_path: str, page_index: int, dpi: int, target_char: Optional[str], reader: Optional[PdfReader] = None, kraken_model: str = '', kraken_bin: str = 'kraken', kraken_python: str = '', allow_livetext: bool = True, facts_sections: Optional[List[str]] = None, facts_mode: str = '', facts_space_index: Optional[dict] = None, alt_ocr: str = "ocrmac_then_kraken") -> Tuple[Dict, List[Dict], bool]:
    ...
    if text_layer_usable(pdf_text):
        ...
        if ok:
            ...
            owner = enrich_owner_with_review(owner, items, pdf_path, page_index+1, dpi=dpi, pil_page=None, facts_sections=facts_sections, facts_mode=facts_mode, reader=reader, facts_space_index=facts_space_index)
    ...
    owner.update(phone_fields)
    owner = enrich_owner_with_review(owner, items, pdf_path, page_index+1, dpi=dpi, pil_page=pil_original, facts_sections=facts_sections, facts_mode=facts_mode, reader=reader, facts_space_index=facts_space_index)
    return owner, items, is_interment
