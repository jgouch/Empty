...
V44 Improvements:
- TRUST BUT VERIFY (from v43): If the Text Layer (PyPDF2) fails to yield a valid name
...
def get_header_candidate(lines: List[str], addr_idx: Optional[int], target_char: Optional[str], aggressive: bool) -> List[str]:
    ...
    search_iter = range(len(search_lines) - 1, -1, -1) if (addr_idx is not None and addr_idx > 0) else range(len(search_lines))
    ...
    for i in search_iter:
        ...
        if addr_idx is not None and addr_idx > 0:
            header.insert(0, ln_clean)
        else:
            header.append(ln_clean)
        if len(header) >= 2:
            break
    return header
...
def parse_owner_header(lines: List[str], target_char: Optional[str] = None) -> Tuple[str, str, str, str, Dict, bool]:
    ...
    header = get_header_candidate(lines, addr_idx, target_char, aggressive=False)
    if not header:
        header = get_header_candidate(lines, addr_idx, target_char, aggressive=True)

    header_text = normalize_ws(" ".join(header))
...
def item_dict_from_line(txt: str, struck: bool = False) -> Dict:
    ...
    prop_details = extract_property_details(txt) if cls["IsProperty"] else {"Garden": "", "Section": "", "Lot": "", "Space": ""}
    return {
        "LineText": txt,
...
# -----------------------------
# PAGE PROCESSOR (Trust-But-Verify)
# -----------------------------
...
def process_page(pdf_path: str, page_index: int, dpi: int, target_char: Optional[str], reader: Optional[PdfReader] = None) -> Tuple[Dict, List[Dict], bool]:
    ...
    if text_layer_usable(pdf_text):
        ...
        # TRUST BUT VERIFY: must start with target_char when target exists
        ok = bool(p) and (not target_char or p.upper().startswith(target_char.upper()))
        if ok:
...
# -----------------------------
# NEIGHBOR CONTEXT + AGGREGATION
# -----------------------------

def apply_neighbor_context(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    for i in range(1, len(df) - 1):
        name = "" if pd.isna(df.at[i, "PrimaryOwnerName"]) else str(df.at[i, "PrimaryOwnerName"])
...
def process_dataset(pdf_path: str, out_path: str, dpi: int = 300):
    ...
    owners_rows, items_rows, interment_rows = [], [], []
...
def main():
    ...
    if not pdf_files:
        ...
        if not pdf_files:
            print("❌ No '* (all).pdf' files found next to the script or in the current folder.")
            return
