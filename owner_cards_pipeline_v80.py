...
Run:
  python3 owner_cards_pipeline_v80.py
  or
  python3 owner_cards_pipeline_v80.py --pdf "B (all).pdf" --out "OwnerCards_B_Output.xlsx"
...
def enrich_owner_with_review(owner: Dict, items: List[Dict], pdf_path: str, page_num: int, dpi: int, pil_page: Optional[Image.Image] = None, facts_sections: Optional[List[str]] = None, facts_mode: str = '', reader: Optional[PdfReader] = None, facts_space_index: Optional[dict] = None, facts_owner_index: Optional[dict] = None) -> Dict:
    ...
    for it in (items or []):
        try:
            ...
            if (not sec_name) and facts_space_index and lotsec and sp:
            ...
            if facts_space_index and lotsec and sp:
    ...
    if property_lines_for_validation:
        ...
        if facts_sections:
            for ln in property_lines_for_validation:
                ...
                if (not sec) and facts_space_index:
...
def has_strong_property_locator(text: str) -> bool:
    """True only when the line contains a strong location signal (not just a garden word)."""
    if not text:
        return False
    u = str(text).upper()
    try:
...
def process_page(pdf_path: str, page_index: int, dpi: int, target_char: Optional[str], reader: Optional[PdfReader] = None, kraken_model: str = '', kraken_bin: str = 'kraken', kraken_python: str = '', allow_livetext: bool = True, facts_sections: Optional[List[str]] = None, facts_mode: str = '', facts_space_index: Optional[dict] = None, facts_owner_index: Optional[dict] = None, alt_ocr: str = "ocrmac_then_kraken") -> Tuple[Dict, List[Dict], bool]:
    ...
    if text_layer_usable(pdf_text):
        ...
        if ok:
            ...
            owner = enrich_owner_with_review(owner, items, pdf_path, page_index+1, dpi=dpi, pil_page=None, facts_sections=facts_sections, facts_mode=facts_mode, reader=reader, facts_space_index=facts_space_index, facts_owner_index=facts_owner_index)
    ...
    owner.update(phone_fields)
    owner = enrich_owner_with_review(owner, items, pdf_path, page_index+1, dpi=dpi, pil_page=pil_original, facts_sections=facts_sections, facts_mode=facts_mode, reader=reader, facts_space_index=facts_space_index, facts_owner_index=facts_owner_index)
