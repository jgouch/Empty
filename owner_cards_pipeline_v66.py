#!/usr/bin/env python3
"""Owner Card Pipeline — v66 (FaCTS Section Validation + Full Snapshots + NeedsReviewNotes)
...
Run:
  python3 owner_cards_pipeline_v66.py
  or
  python3 owner_cards_pipeline_v66.py --pdf "B (all).pdf" --out "OwnerCards_B_Output.xlsx"
...
# v66: base directory for snapshot outputs (set to PDF folder during processing)
SNAPSHOT_BASE_DIR = ""
    ...
    if needs_review:
        try:
            ...
            if pil_page is not None:
                ...
                owner['SnapshotItems'] = ''
                owner['SnapshotRotate'] = '0'
                owner['SnapshotRotateScore'] = '0'
        except Exception:
            pass
    else:
        owner['SnapshotFull'] = ''
...
def save_failure_snapshots(pil_img: Image.Image, source_pdf: str, page_num: int, reason: str = "NEEDS_REVIEW") -> Dict[str, str]:
    """Save ONLY a full-page snapshot for manual verification (no rotation, no crops)."""
    ...
    try:
        pil_img.save(full_path, quality=92)
    except Exception:
        pil_img.convert('RGB').save(full_path, quality=92)
...
def preprocess_ghost(pil_img: Image.Image) -> Image.Image:
    ...
    bin_img = cv2.erode(bin_img, kernel, iterations=1)  # v66: thicken faint ink
...
def preprocess_high_contrast(pil_img: Image.Image) -> Image.Image:
    """v66: high-contrast preprocessor for yellowed paper/light typing."""
    try:
        img_bgr = cv2.cvtColor(np.array(pil_img), cv2.COLOR_RGB2BGR)
        ...
        bin_img = cv2.adaptiveThreshold(sharp, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 35, 9)
        bin_img = ensure_dark_text_on_white(bin_img)
        return Image.fromarray(bin_img)
    except Exception:
        return preprocess_shadow_correct(pil_img)


def normalize_for_ocr(pil_img: Image.Image) -> Tuple[Image.Image, Dict]:
    """v66: apply OSD auto-rotation + deskew before OCR. Returns (image, meta)."""
    meta = {'osd_rotate': 0, 'osd_conf': 0, 'deskew_angle': 0.0, 'osd_applied': False, 'deskew_applied': False}
    if pil_img is None:
        return pil_img, meta
    img = pil_img
    # OSD rotation
    try:
        osd = pytesseract.image_to_osd(preprocess_high_contrast(img))
    ...
    try:
        ...
        if coords.size > 0:
            ...
            if abs(angle) >= 0.6:
                h, w = bgr.shape[:2]
                M = cv2.getRotationMatrix2D((w // 2, h // 2), angle, 1.0)
                bgr2 = cv2.warpAffine(bgr, M, (w, h), flags=cv2.INTER_CUBIC, borderMode=cv2.BORDER_REPLICATE)
                img = Image.fromarray(cv2.cvtColor(bgr2, cv2.COLOR_BGR2RGB))
...
def should_apply_osd_deskew(pdf_text: str, text_layer_ok: bool, primary_name: str, addr: Dict, phone_fields: Dict, items: List[Dict], header_ocr_meta: Optional[Dict] = None) -> Tuple[bool, str]:
    """v66: decide whether to run OSD+deskew normalization BEFORE OCR."""
    if not text_layer_ok:
        return True, 'OCR_FALLBACK_PAGE'
    u_primary = (primary_name or '').upper()
    if '[MISSING' in u_primary or len(u_primary.strip()) < 3:
        return True, 'HEADER_NAME_MISSING'
    if not (addr.get('State') or '').strip() or not (addr.get('ZIP') or '').strip():
        return True, 'ADDRESS_INCOMPLETE'
    if not phone_fields.get('PhoneValid', False) and not (phone_fields.get('PhoneRaw') or phone_fields.get('PhoneNormalized')):
        return True, 'PHONE_MISSING'
    if pdf_text:
        t = pdf_text.upper()
        seems_like_has_items = ('ITEM DESCRIPTION' in t) or ('OWNER ID' in t) or ('LOT' in t) or ('SEC' in t) or ('SP' in t)
        has_included = any(bool(it.get('Include')) for it in (items or []))
        if seems_like_has_items and not has_included:
            return True, 'ITEMS_EMPTY_SUSPECT'
    if header_ocr_meta and isinstance(header_ocr_meta, dict):
        try:
            sc = int(header_ocr_meta.get('score', 0))
            if sc and sc < 25:
                return True, 'HEADER_OCR_LOW_SCORE'
        except Exception:
            pass
    return False, ''
...
def process_page(pdf_path: str, page_index: int, dpi: int, target_char: Optional[str], reader: Optional[PdfReader] = None, kraken_model: str = '', kraken_bin: str = 'kraken', kraken_python: str = '', allow_livetext: bool = True, facts_sections: Optional[List[str]] = None, facts_mode: str = '', alt_ocr: str = "ocrmac_then_kraken") -> Tuple[Dict, List[Dict], bool]:
    ...
    if text_layer_usable(pdf_text):
        ...
        if ok:
            ...
            # v66: conditional OSD/deskew + normalized header OCR salvage (only when needed)
            try:
                apply_norm, norm_reason = should_apply_osd_deskew(txt, True, p, addr, phone_fields, items, owner_header_meta if 'owner_header_meta' in locals() else None)
                if apply_norm:
                    pil_norm = render_page_deskew(pdf_path, page_index, dpi=min(dpi, 300))
                    if pil_norm is not None:
                        pil_norm, ocr_norm_meta2 = normalize_for_ocr(pil_norm)
                        htxt, hmeta = ocr_header_ensemble(pil_norm, kraken_model=kraken_model, kraken_bin=kraken_bin, kraken_python=kraken_python, allow_livetext=allow_livetext, alt_ocr=alt_ocr)
                        hlines = split_lines(htxt)
                        ...
                        pphone = extract_phone_fields(htxt, hlines)
                        if pphone.get('PhoneValid') and pphone.get('PhoneNormalized'):
                            phone_fields.update(pphone)
                        owner_header_meta = hmeta
                        # store norm diagnostics into owner later
                        _norm_diag = (ocr_norm_meta2, norm_reason)
                    else:
                        _norm_diag = (None, norm_reason)
                else:
                    _norm_diag = (None, '')
            except Exception:
                _norm_diag = (None, '')
            owner = {
                ...
                "OCR_OSD_Conf": 0,
                "OCR_DeskewAngle": 0.0,
                "OCR_OSD_Applied": False,
                "OCR_DeskewApplied": False,
                "OCR_NormReason": '',
            }
            ...
            try:
                _m, _r = _norm_diag
                if _m:
                    owner['OCR_OSD_Rotate'] = _m.get('osd_rotate', 0)
                    owner['OCR_OSD_Conf'] = _m.get('osd_conf', 0)
                    owner['OCR_DeskewAngle'] = _m.get('deskew_angle', 0.0)
                    owner['OCR_OSD_Applied'] = bool(_m.get('osd_applied', False))
                    owner['OCR_DeskewApplied'] = bool(_m.get('deskew_applied', False))
                    owner['OCR_NormReason'] = _r
            except Exception:
                pass
    ...
    pil_original, ocr_norm_meta = normalize_for_ocr(pil_original)
    ...
    owner = {
        ...
        "HeaderOCRUsedKraken": bool(header_meta.get('kraken')) if isinstance(header_meta, dict) else False,
            "OCR_OSD_Rotate": ocr_norm_meta.get('osd_rotate', 0),
            "OCR_OSD_Conf": ocr_norm_meta.get('osd_conf', 0),
            "OCR_DeskewAngle": ocr_norm_meta.get('deskew_angle', 0.0),
            "OCR_OSD_Applied": bool(ocr_norm_meta.get('osd_applied', False)),
            "OCR_DeskewApplied": bool(ocr_norm_meta.get('deskew_applied', False)),
            "OCR_NormReason": 'OCR_FALLBACK_PAGE',
    }
    owner.update(phone_fields)
...
def process_dataset(pdf_path: str, out_path: str, dpi: int = 300, kraken_model: str = '', kraken_bin: str = 'kraken', kraken_python: str = '', allow_livetext: bool = True, facts_path: str = '', alt_ocr: str = "ocrmac_then_kraken"):
    ...
    # v66: write snapshots next to the input PDF folder
