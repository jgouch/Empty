...
def save_failure_snapshot(pil_img: Image.Image, source_pdf: str, page_num: int, reason: str = "MISSING") -> str:
    """Save a quick header crop (top ~32%) for rapid manual review."""
...
def try_kraken_text(pil_img: Image.Image, model_path: str = "", kraken_bin: str = "kraken") -> str:
    """Last-resort OCR using Kraken HTR/OCR via CLI. Returns text or ''."""
    if not model_path:
        return ""
    import tempfile
    import subprocess
