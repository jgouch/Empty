#!/usr/bin/env python3
"""
Owner Card Pipeline (Local) — Excel-Safe + Excel-Length-Safe (Final Hardened v28)
...
V28 Improvements:
- THE NAME HUNTER: If primary name extraction fails, triggers a deep regex search for "Lastname, Firstname".
- PREFIX SURGEON: Smartly strips "y-", "'-AN", "4¢" by cleaning words individually before checking the Target Letter.
- GIBBERISH FILTER 2.0: Stricter checks to prevent "AIS FUN LLY" from being accepted.
- RESTORED: All Excel safety functions and duplicate checking.
"""
...
def is_gibberish(text: str) -> bool:
    ...
    # If no vowels, likely garbage
    if not re.search(r'[AEIOUYaeiouy]', text): return True
    # If too many single char words
    words = text.split()
...
def clean_name_line(line: str, target_char: Optional[str] = None) -> str:
    """
    V28: SURGICAL PREFIX STRIPPER
    Cleans individual words of non-alpha chars before checking target char.
    """
    if not line: return ""
    ...
    if target_char:
        ...
        for i, w in enumerate(words):
            # Surgical Clean: Remove leading non-alpha (e.g. 'y-', '4¢', '(', "'")
            # But keep the rest of the word
            w_surgical = re.sub(r"^[^a-zA-Z]+", "", w)
            
            # Check 1: Does the surgically cleaned word start with Target?
            if w_surgical.upper().startswith(tc):
                cleaned_words.append(w_surgical) # Add the cleaned word
                cleaned_words.extend(words[i+1:]) # Add rest of line
                found_start = True
                break
                
            # Check 2: Fuzzy Match (OCR errors)
            if len(w_surgical) > 3:
                first_char = w_surgical[0].upper()
                if first_char in ['B', 'O', '4', '@', 'F'] and tc == 'A':
                    w_corrected = 'A' + w_surgical[1:]
