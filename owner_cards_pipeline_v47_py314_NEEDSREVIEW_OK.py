...
# ---------------------------------
# REVIEW / SALVAGE HELPERS
# ---------------------------------
def extract_state_zip_anywhere(text: str) -> Tuple[str, str]:
    """Best-effort: find ZIP and adjacent state anywhere in text (used for OCR fallback salvage)."""
...
def process_page(pdf_path: str, page_index: int, dpi: int, target_char: Optional[str], reader: Optional[PdfReader] = None) -> Tuple[Dict, List[Dict], bool]:
    ...
    if text_layer_usable(pdf_text):
        ...
        if ok:
            ...
            # NeedsReview: salvage what we can, but flag rows that should be manually verified
    ...
    # Attempt salvage of State/ZIP anywhere in OCR text if address parsing failed
    if (not addr.get("State")) and (not addr.get("ZIP")):
    ...
    # Attempt salvage of a comma-style name near the top if name looks missing
